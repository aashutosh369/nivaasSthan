...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    let user = await userModel.findById(req.userId).select("-password");// agar userId mill gyi to db ke userModel se userID ke through user details ko find kr ke user varriable me store kr na... 
    if (!user) {
